  ...
  cast_owner_output="$(
    awk '
...

        {
          ...
          if (line ~ /\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]+[^)]*Type[[:space:]]*\)[[:space:]]*\.[[:space:]]*spatial(Read|Write|MaintenanceWrite)[[:space:]]*=/) {
            print NR
            state = "none"
            start_line = 0
            next
          ...
          if (state == "cast_wait_type") {
            ...
            if (line ~ /^[[:space:]]*[^)]*Type[[:space:]]*\)[[:space:]]*\.[[:space:]]*spatial(Read|Write|MaintenanceWrite)[[:space:]]*=/) {
              print start_line
              state = "none"
              start_line = 0
              next
          ...
          if (line ~ /^[[:space:]]*\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]+[^)]*Type[[:space:]]*\)[[:space:]]*$/) {
            state = "owner_ready"
            start_line = NR
            next
          }

          if (line ~ /^[[:space:]]*\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]*$/) {
  ...
  nested_cast_owner_output="$(
    awk '
...

        {
          ...
          if (line ~ /\([[:space:]]*\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]+[^)]*Type[[:space:]]*\)[[:space:]]*\)[[:space:]]*\.[[:space:]]*spatial(Read|Write|MaintenanceWrite)[[:space:]]*=/) {
            print NR
            state = "none"
            start_line = 0
            next
          ...
          if (state == "nested_cast_wait_type") {
            ...
            if (line ~ /^[[:space:]]*[^)]*Type[[:space:]]*\)[[:space:]]*\)[[:space:]]*\.[[:space:]]*spatial(Read|Write|MaintenanceWrite)[[:space:]]*=/) {
              print start_line
              state = "none"
              start_line = 0
              next
            ...
            if (line ~ /^[[:space:]]*[^)]*Type[[:space:]]*\)[[:space:]]*\)[[:space:]]*$/) {
              state = "owner_ready"
              next
            }
          ...
          if (line ~ /^[[:space:]]*\([[:space:]]*\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]+[^)]*Type[[:space:]]*\)[[:space:]]*\)[[:space:]]*$/) {
            state = "owner_ready"
            start_line = NR
            next
          }

          if (line ~ /^[[:space:]]*\([[:space:]]*\([[:space:]]*(((self|Self|DataContractRegistry)|([A-Za-z_][A-Za-z0-9_]*)|(`([[:space:]]*[A-Za-z_][A-Za-z0-9_]*[[:space:]]*)`))([[:space:]]*\.[[:space:]]*self)?)[[:space:]]+as[[:space:]]*$/) {
...
echo "Data contract/domain boundaries passed (no forbidden platform imports/runtime symbols, no Realm adapter seam leaks, constructor wiring boundaries preserved including registry-default declarations, registry spatial-adapter assignment seams preserved including parenthesized/multiline/split-member/parenthesized-owner/parenthesized-alias-owner/nested-parenthesized-owner/cast-owner/nested-cast-owner/spaced-member/comment-interleaved/block-comment-separated/inout/key-path/metatype-alias/typealias/typealias-chain/typealias-derived-metatype-alias/escaped-alias-owner/escaped-owner/escaped-identifier wiring detection, and test-only registry overrides)."
