...
- `SSGeo` now lives in `apps/common` and provides portable location payloads, heading/quadrant/path helpers, centroid calculations, Web-Mercator projection helpers, and basic geodesic math without `CoreLocation`.
...
- `SSDataContracts` now lives in `apps/common` and hosts shared contract-side value types for storage/read-write boundaries, including universal-link parameter/parsing types, `VectorTile`, the Swift `GDAJSONObject` helper, and the raw GeoJSON geometry parser/types after decoupling them from iOS runtime behavior.
...
- Imported `LocationParameters` entity materialization now goes through `DataContractRegistry.spatialMaintenanceWrite.materializePointOfInterest(from:)`; do not reintroduce direct serialization-layer calls to `GDASpatialDataResultEntity.addOrUpdateSpatialCacheEntity(...)`.
- `MarkerParameters(markerId:)`, `MarkerParameters(entity:)`, and route-parameter serialization now resolve persisted marker data through async `DataContractRegistry.spatialRead` lookups; do not reintroduce synchronous `LocationDetailStoreAdapter` lookups in serialization code.
- Serialization should determine shareable entity payloads through protocol-based POI capabilities rather than direct casts to infrastructure types like `GDASpatialDataResultEntity`.
- `LocationDetail.updateLastSelectedDate()` now routes marker/POI selection writes through `DataContractRegistry.spatialWrite`; do not reintroduce direct `LocationDetailStoreAdapter` selection writes from non-infrastructure code.
- Route sharing should serialize from the already-loaded `RouteDetail`/`LocationDetail` surface when possible instead of synchronously re-querying persistence for share payload assembly.
- `RouteDetail` waypoint hydration for database/cache-backed routes now resolves through async `DataContractRegistry.spatialRead` lookups; do not add new route UI paths that depend on `RouteWaypoint.asLocationDetail` or `LocationDetail(markerId:)` for production persistence reads.
- Route first-waypoint fallback coordinate resolution and imported-route waypoint comparison should use persisted marker IDs or imported `ReferenceEntity` coordinates directly; do not reintroduce `RouteWaypoint.asLocationDetail` for those helpers.
- `RouteWaypoint.asLocationDetail` has been removed, and the sync `LocationDetail.init?(markerId:)` / `init?(entityId:)` loaders are deprecated compatibility seams; new persisted detail loading should use the async `LocationDetail.load(...)` entry points.
- Search/nearby/home/current-location flows should prefer the async `LocationDetail.load(entity:)` / `LocationDetail.load(location:)` builders so marker state is pre-resolved through `DataContractRegistry.spatialRead` instead of falling back to sync `LocationDetail` cache/entity lookups later.
...
- `GeometryType`, the raw GeoJSON coordinate-shape aliases, and the shared `GeoJSONGeometryParser` now live in `apps/common/Sources/SSDataContracts`; keep `GeometryUtils` and `GeoJsonGeometry` in `apps/ios` only as compatibility wrappers over that shared parser plus app-specific model behavior.
...
- `Quadrant`, `CompassDirection`, the shared heading-to-quadrant bucketing logic, the portable path/centroid helpers, and the shared Web-Mercator projection/closest-edge/polygon-containment math now live in `apps/common/Sources/SSGeo`; keep `GeometryUtils` and `VectorTile` as wrappers only where app- or contract-specific compatibility is still required.
