...
`process(_:)` now behaves asynchronously: events are yielded into an `AsyncStream` that a main-actor task consumes before invoking the active behavior. This removes the recursive delegate pyramid that previously re-entered `process(_:)` synchronously, keeps event ordering deterministic, and lays the groundwork for letting behaviors subscribe to the stream directly in a future refactor.
