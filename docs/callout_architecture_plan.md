...
- **Intersection location updates via subscription:** `IntersectionGenerator` now consumes `LocationUpdatedEvent` via its state stream subscription (state-only processing that may enqueue follow-on events) so the legacy `handle(event:)` stays focused on callout-producing events.
