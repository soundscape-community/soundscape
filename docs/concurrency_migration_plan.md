...
- 2025-11-28: Callout coordinator takeover - Inlined the remaining hush/queue/playback logic from `CalloutStateMachine` into `CalloutCoordinator`, deleted the legacy file/project references, rewired `AppContext`/`EventProcessor`/tests, and confirmed the coordinator is now the single playback surface.
- 2025-11-28: Event stream queue - Reworked `EventProcessor.process` to enqueue events into an `AsyncStream` that a main-actor task consumes, eliminating recursive delegate chains and setting up the infrastructure needed for behavior-specific subscriptions. Added a unit test to assert that queueing still dispatches events.

---
Last updated: 2025-11-28 (Tasks 5 & 8 in progress: AudioEngine hops trimmed; SpatialDataContext/DeviceReachability/URLResourceManager audits complete; static constant warnings removed; CalloutCoordinator now owns playback; EventProcessor now queues events via AsyncStream; 49/49 tests passing)
