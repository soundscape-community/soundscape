...
- `DestinationManager` no longer exposes concrete keyed destination POI reads (`destinationPOI(forReferenceID:)`); destination POI lookup now remains internal to destination storage operations (`DestinationEntityStore`) while app/runtime callers continue using destination ID/entity-key metadata seams plus contract ingress.
- `DestinationManagerProtocol` no longer requires keyed destination POI reads (`destinationPOI(forReferenceID:)`) for protocol-driven callers; infrastructure destination context in `SpatialDataContext` now resolves active destination POI via keyed entity lookup (`destinationEntityKey(forReferenceID:)` + `searchByKey`) with `referenceEntityByKey(...).getPOI()` fallback.
...
- `DestinationManagerProtocol` now exposes keyed destination nickname lookup (`destinationNickname(forReferenceID:)`), and destination nickname reads in `AppContext` and `DestinationTutorialPage` now use keyed destination ID reads instead of active-destination metadata properties.
