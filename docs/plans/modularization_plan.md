...
- `RouteWaypoint.asLocationDetail` has been removed, and the sync `LocationDetail` marker/entity ID initializers are now explicitly deprecated in favor of the async `LocationDetail.load(...)` entry points.
...
- The deprecated sync `LocationDetail.init?(markerId:)` / `init?(entityId:)` compatibility loaders are now fenced to debug-only preview/test code, and the last preview call site no longer depends on them for marker detail construction.
...
- Deleted the now-unused `RouteWaypoint.asLocationDetail` compatibility property/array helper, and marked the sync `LocationDetail.init?(markerId:)` / `init?(entityId:)` loaders as deprecated so only preview/test compatibility code should remain on those paths.
...
- Fenced the deprecated sync `LocationDetail` marker/entity ID loaders to debug-only compatibility code and rewired the remaining `WaypointAddList` preview to construct its sample marker detail directly, leaving no production callers on those sync entry points.
...
## Next Steps
1. Remove the remaining debug-only deprecated sync `LocationDetail.init?(markerId:)` / `init?(entityId:)` compatibility loaders once preview/test fixtures have a stable async or direct-fixture replacement everywhere they still need persisted marker/entity details.
