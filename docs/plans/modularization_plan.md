...
Current architecture baseline (latest report `20260304-154602Z-ssindex-94b1388`):
- `Data -> App`: 678
- `Data -> Visual UI`: 94
...
- Dependency metrics now have a fresh normalized rerun (`20260304-154602Z-ssindex-94b1388`); follow-up deltas should be compared against that artifact with fixed args.
...
1. Use normalized SSIndex baseline `20260304-154602Z-ssindex-94b1388` for Milestone 3 delta tracking; keep analyzer args fixed (`--top 40 --min-count 2 --file-top 40 --external-top 25`) on each follow-up export.
2. Execute the next Milestone 3 hardening slice for adapter wiring beyond constructor/registry-default/assignment-scope seams, keeping coverage comparable to existing symbol-boundary checks.
...
- Milestone 3 now includes constructor, registry-default, assignment-shape, and assignment-method-scope guardrails for `DataContractRegistry` Realm adapter wiring seams.
...
- `docs/plans/artifacts/dependency-analysis/20260304-154602Z-ssindex-94b1388.txt`
...
2. Continue Milestone 3 adapter-isolation hardening slices (constructor, registry-default wiring, registry assignment-shape, and registry assignment-method-scope guardrails landed; proceed with next wiring-boundary hardening slice).
