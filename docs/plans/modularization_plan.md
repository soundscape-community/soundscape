...
- 2026-02-26: `check_data_contract_infra_type_allowlist.sh` now treats `Road` and `Route` as canonical app-facing domain types (not Realm infrastructure types), shrinking temporary `Data/Contracts` infrastructure leakage monitoring to `Intersection` and `TileData`.
- 2026-02-26: Contract allowlist refinement validation is green for `check_data_contract_infra_type_allowlist.sh` (detected contract-side infrastructure types: `Intersection`, `TileData`).
...
3. Keep `SpatialDataStoreRegistry.store` guardrails strict (`Data/Infrastructure/Realm/**` only, allowlist empty) and continue shrinking the remaining temporary `Data/Contracts` infrastructure-type allowlist entries (`Intersection`, `TileData`) as each contract surface is replaced with domain/value equivalents.
