...
## Next Steps
1. Continue migrating the remaining `LocationDetail(entity:)` / raw-location construction call sites onto the async `LocationDetail.load(entity:)` / `load(location:)` builders, then return to the remaining internal sync fallbacks inside `LocationDetail` (`Source.entity`, destination-marker matching, and `referenceEntity(source:)`).
