...
## Next Steps
1. Continue auditing the remaining `LocationDetail` read paths inside `LocationDetail` itself (`Source.entity`, destination-marker matching, and `referenceEntity(source:)`) so non-infrastructure UI code stops depending on `LocationDetailStoreAdapter` for persistence reads.
