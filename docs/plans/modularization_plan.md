...
- 2026-02-10: Added `check_data_contract_infra_type_allowlist.sh` and wired it into CI to enforce a temporary explicit allowlist for Realm infrastructure model type names that may appear in `Data/Contracts`, preventing new type leakage while DTO migration continues.
...
- 2026-02-10: Migrated marker-heavy read surfaces in `SpatialDataView`, `RoadAdjacentDataView` marker-path helpers, and `ExplorationGenerator` from `RealmReferenceEntity` arrays to domain `ReferenceEntity` values while preserving existing read flow through deprecated compatibility seams.
- 2026-02-10: Added domain behavior parity helpers on `ReferenceEntity` (`name`, `givenLocalizedName`, `address`, `displayAddress`, `distanceToClosestLocation`, `bearingToClosestLocation`, `closestLocation`, `getPOI`) plus `GenericLocation.init(ref: ReferenceEntity)` so migrated callers no longer depend on Realm object methods for read-only marker behavior.
- 2026-02-10: Migrated marker sorting helpers to domain entities (`Array+Extensions.sort/selectNearestInEachQuadrant`, `MarkerQueue`) and updated `SpatialDataViewProtocol.markedPoints` to `[ReferenceEntity]` so behavior-layer marker callout selection uses domain types end-to-end on this slice.
- 2026-02-10: Validation for domain-marker read slice: `check_data_contract_boundaries.sh`, `check_data_contract_infra_type_allowlist.sh`, `check_realm_infrastructure_boundary.sh`, and `check_spatial_data_cache_seam.sh` pass; `xcodebuild build-for-testing` passes; targeted suites `DestinationManagerTest`, `RouteStorageProviderDispatchTests`, `DataContractRegistryDispatchTests`, and `CloudSyncContractBridgeTests` pass; `swift Scripts/LocalizationLinter/main.swift` passes.
- 2026-02-10: Migrated `POICalloutProtocol`, `POICallout`, and `DestinationCallout` marker access from `RealmReferenceEntity` to domain `ReferenceEntity`, and switched POI/marker lookups in those callouts from direct `SpatialDataCache` reads to `DataContractRegistry.spatialReadCompatibility` seams.
- 2026-02-10: Validation for callout-domain marker slice: boundary scripts pass, `xcodebuild build-for-testing` passes, targeted suites `CalloutCoordinatorTests`, `DestinationManagerTest`, `RouteStorageProviderDispatchTests`, `DataContractRegistryDispatchTests`, and `CloudSyncContractBridgeTests` pass, and `swift Scripts/LocalizationLinter/main.swift` passes.
- 2026-02-10: Migrated `LocationDetail` marker reads to domain `ReferenceEntity` (while preserving source-based temp-marker lookup semantics), switched marker-id/entity-id POI resolution to `DataContractRegistry.spatialReadCompatibility`, and added a domain-marker initializer path for `LocationDetail`.
- 2026-02-10: Migrated additional UI marker read callsites to contract/domain seams (`MarkerEditViewRepresentable`, `ShareMarkerAlertObserver`, `BeaconActionHandler`, `BeaconDetailStore`, `POITableViewCellConfigurator`, `MarkerCell`, `MarkersList`, `SearchResultsTableViewController`, `LocationActionHandler`) so these paths no longer depend on direct `SpatialDataCache.referenceEntityBy*` reads.
- 2026-02-10: Validation for location-detail/ui marker domain slice: boundary scripts pass, `xcodebuild build-for-testing` passes, targeted suites `CalloutCoordinatorTests`, `DestinationManagerTest`, `RouteStorageProviderDispatchTests`, `DataContractRegistryDispatchTests`, and `CloudSyncContractBridgeTests` pass, and `swift Scripts/LocalizationLinter/main.swift` passes.
- 2026-02-10: Removed remaining direct `SpatialDataCache.referenceEntityBy*` reads outside infrastructure by migrating tutorial marker pages (`DestinationTutorialPage`, `MarkerTutorialViewController`) and behavior marker lookups (`PreviewBehavior`, `AutoCalloutGenerator`) to `DataContractRegistry.spatialReadCompatibility`.
- 2026-02-10: Replaced `EditMarkerView` temporary-marker fallback from `SpatialDataCache.referenceEntity(source:isTemp:)` to contract-backed source resolution with explicit `isTemp` filtering to preserve previous update semantics while reducing direct cache coupling.
- 2026-02-10: Validation for tutorial/behavior marker-read migration slice: `check_data_contract_boundaries.sh`, `check_data_contract_infra_type_allowlist.sh`, `check_realm_infrastructure_boundary.sh`, and `check_spatial_data_cache_seam.sh` pass; `xcodebuild build-for-testing` passes; targeted suites `DestinationManagerTest`, `RouteStorageProviderDispatchTests`, `DataContractRegistryDispatchTests`, and `CloudSyncContractBridgeTests` pass; `swift Scripts/LocalizationLinter/main.swift` passes.
- 2026-02-10: Replaced the final non-infrastructure source-based marker cache read in `LocationDetail` (`SpatialDataCache.referenceEntity(source:isTemp:)`) with contract-backed source resolution that preserves prior source semantics (`.designData` no marker match, optional `isTemp` filter).
...
- 2026-02-10: Extended data write contracts with marker-focused mutation APIs (`addReferenceEntity(entityKey:...)`, `addReferenceEntity(location:...)`, `updateReferenceEntity(id:...)`) and implemented Realm adapter support in `RealmSpatialWriteContract` (id-based update lookup + existing Realm mutation path delegation).
...
- 2026-02-10: Replaced the temporary `Address` contract payload leak with app-facing `AddressCacheRecord` value shape for `restoreCachedAddresses`, keeping settings cache restore seam functionality while removing direct infrastructure type usage from write-contract signatures.
- 2026-02-10: Updated `StatusTableViewController` address preservation to emit `AddressCacheRecord` values and adjusted `RealmSpatialWriteContract` to map those records back into `Address` objects inside infrastructure boundaries.
- 2026-02-10: Removed `Address` from `check_data_contract_infra_type_allowlist.sh` allowed contract type list after converting the seam to `AddressCacheRecord`.
- 2026-02-10: Validation for `AddressCacheRecord` seam cleanup slice: `check_data_contract_boundaries.sh`, `check_data_contract_infra_type_allowlist.sh`, `check_realm_infrastructure_boundary.sh`, and `check_spatial_data_cache_seam.sh` pass; `xcodebuild build-for-testing` passes; targeted suites `CalloutCoordinatorTests`, `DestinationManagerTest`, `RouteStorageProviderDispatchTests`, `DataContractRegistryDispatchTests`, and `CloudSyncContractBridgeTests` pass; `swift Scripts/LocalizationLinter/main.swift` passes.
- 2026-02-10: Extended write contracts with route mutation APIs (`addRoute`, `updateRoute`, `deleteRoute`) across async and deprecated compatibility surfaces, with Realm adapter support in `RealmSpatialWriteContract`.
