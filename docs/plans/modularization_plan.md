...
## Progress Updates
- 2026-03-07: Removed async `removeAllRoutes` dependence on sync `Route.deleteAll()` store scans; `RealmSpatialMaintenanceWriteContract.removeAllRoutes()` now enumerates route IDs through `DataContractRegistry.spatialRead.routes()` before invoking route deletes, with dispatch coverage asserting no `SpatialDataStoreRegistry.store.routes()` fallback usage.
