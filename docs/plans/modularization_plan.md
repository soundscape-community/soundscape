...
## Next Steps
1. Continue migrating the remaining runtime `LocationDetail(entity:)` / raw-location construction call sites that still sit on sync-only presentation/configuration paths, then return to the remaining internal sync fallbacks inside `LocationDetail` (`Source.entity`, destination-marker matching, and `referenceEntity(source:)`).
