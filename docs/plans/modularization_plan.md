...
Current architecture baseline (latest report `20260304-112509Z-ssindex-2acf565`):
- `Data -> App`: 678
- `Data -> Visual UI`: 94
...
1. Continue Milestone 4 by expanding in-memory parity coverage for remaining maintenance/read-write edges (for example remove-all flows and route/reference side effects around cleanup/import paths).
...
- Milestone 2 is complete; Milestone 3 hardening is active and Milestone 4 parity expansion now includes maintenance semantics (`clearNewReferenceEntitiesAndRoutes` and corrupt-reference cleanup lookup behavior).

Latest dependency artifact:
- `docs/plans/artifacts/dependency-analysis/20260304-112509Z-ssindex-2acf565.txt`
