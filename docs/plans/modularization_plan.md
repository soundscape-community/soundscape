...
1. Continue Milestone 1 seam carving in remaining high-coupling UI view controllers under `Visual UI` (`DevicesViewController`, `HomeViewController`, `MarkerTutorialViewController`) by introducing focused runtime-provider hooks rather than passing `AppContext` directly.
