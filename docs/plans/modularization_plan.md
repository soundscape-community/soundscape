...
## Next Steps
1. Continue auditing the remaining `LocationDetail` read paths (`Source.entity`, destination-marker matching, and the sync `entityId`/marker-id loaders) so non-infrastructure UI code stops depending on `LocationDetailStoreAdapter` for persistence reads.
