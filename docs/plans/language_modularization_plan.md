...
3. Audit the remaining `directions.traveling.*`, `directions.facing.*`, `directions.heading.*`, and `directions.along.*` helpers next; they are the most likely remaining runtime-neutral phrase families if further extraction is worthwhile.
